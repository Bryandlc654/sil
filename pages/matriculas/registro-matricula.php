...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['operaciones'])) {
        ...
        if ($_POST['operaciones'] == 'Registrar') {
            if ($conexion) {
                $existeUsuario = $conexion->query("SELECT COUNT(*) AS total FROM Usuarios WHERE Usuario_id = '$idUsuario'");
                $fila = $existeUsuario->fetch_assoc();

                if ($fila['total'] > 0) {
                    $consulta_total = $conexion->query("SELECT COUNT(*) as total FROM Matriculas");
                    $total_registros = $consulta_total->fetch_assoc()['total'];

                    $nuevo_codigo = 'msil-' . str_pad($total_registros + 1, 3, '0', STR_PAD_LEFT);

                    // El usuario existe, procede con la inserción en Matriculas
                    $consulta = $conexion->query("INSERT INTO Matriculas(Matricula_Codigo,Curso_Id, Usuario_Id, Matricula_Fecha) VALUES('$nuevo_codigo','$curso','$idUsuario','$fcreacion')");
                    if ($consulta) {
                        $mensaje = "✅ Registrado";
                        $dni = '';
                        $fcreacion = '';
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    <h4 class="card-title">Registrar Matrícula</h4>
                                    <div class="d-flex ">
