...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $codigomodulo = isset($_POST['codigomodulo']) ? $_POST['codigomodulo'] : '';


    if (isset($_POST['operaciones'])) {
        if ($_POST['operaciones'] == 'Buscar') {
            if ($conexion) {
                $consulta = $conexion->query("SELECT * FROM Modulos WHERE Modulo_Codigo = '$codigo'");

                if ($datos = $consulta->fetch_assoc()) {
                    $modulo = $datos['Modulo_nombre'];
                    $idcurso = $datos['Curso_id'];
                    $codigomodulo = $datos['Modulo_Codigo'];
                }
            }
        }
        if ($_POST['operaciones'] == 'Actualizar') {
            if ($conexion) {
                ...
                UPDATE Modulos
                SET 
                    Modulo_nombre = '$nommodulo',
                    Curso_id='$curso'
                WHERE Modulo_Codigo = '$codigomodulo'");
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    <h4 class="card-title">Actualizar Módulos</h4>
                                </div>
                                <div class="d-flex ">
                                    <form action="" method="post" class="d-flex">
                                        <input type="text" name="codigo" placeholder="Ingresar Codigo del Módulo" class="form-control" />
                                ...
                                <form class="form-sample" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="form-group row" hidden>
                                            ...
                                            <div class="col-sm-9">
                                                <input type="text" class="form-control" name="codigomodulo" value="<?php echo $codigomodulo; ?>" />
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-9">
                                                    <input type="text" class="form-control" name="nommodulo" value="<?php echo $modulo ?>" required />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                                    ...
                                                    <select class="form-control" name="curso" required>
                                                        ...
                                                        if ($resultado = $conexion->query($consulta)) {
                                                            ...
                                                            while ($fila = $resultado->fetch_assoc()) {
                                                                echo '<option value="' . $fila['Curso_id'] . '" ' . ($idcurso == $fila['Curso_id'] ? 'selected' : '') . '>' . $fila['Curso_nombre'] . '</option>';
                                                            }
                                                        } else {
                                                            echo "No existe";
                                                        }
